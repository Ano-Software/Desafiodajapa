    ...
    <main className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100 text-slate-800 flex items-center justify-center px-4">
      <div className="max-w-xl w-full rounded-3xl bg-white border border-slate-200 shadow-lg p-10 space-y-8 text-center">
        ...
        <p className="text-sm text-slate-600 leading-relaxed">
          Este site é exclusivo para registrar a conclusão dos desafios virtuais
          comprados no site{" "}
          <span className="font-semibold text-slate-900">
            superandolimitesofc.com.br
          </span>
          . Para enviar seu comprovante, clique no link de conclusão disponível
          na página do desafio que você comprou.
        </p>
...
export default async function RootPage() {
  ...
  const isConclusionHost =
    host.includes("concluir.desafiodajapa.com.br") ||
    host.startsWith("concluir.") ||
    host.includes("localhost");

  if (isConclusionHost) {
    return <ConclusionFormPage />;
  }
